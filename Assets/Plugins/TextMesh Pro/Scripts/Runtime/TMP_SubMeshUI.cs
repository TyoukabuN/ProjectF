...
namespace TMPro
{
    ...
    public class TMP_SubMeshUI : MaskableGraphic, IClippable, IMaskable, IMaterialModifier
    {
        ...
        public static Material s_GraySubMeshMaterial;
        ...
        protected override void OnEnable()
        {
            ...
            TextMeshProUGUI meshPro = transform.parent.GetComponent<TextMeshProUGUI>();
            if (meshPro !=null)
            {
                s_IsCanSortingMask = meshPro.s_IsCanSortingMask;
            }
            UpdateClipParent();
            //SetAllDirty();
        }
        ...
        {
            m_padding = ShaderUtilities.GetPadding(m_sharedMaterial, isExtraPadding, isUsingBold, m_TextComponent);
        }
        ...
        public void SetGray(bool gray)
        {
            ...
            if (gray && !m_IsGray)
            {
                if (!m_CacheMaterial)
                {
                    m_CacheMaterial = CreateMaterialInstance(m_sharedMaterial);
                }
                if (!s_GraySubMeshMaterial)
                {
                    s_GraySubMeshMaterial = CreateMaterialInstance(m_sharedMaterial); 
                    s_GraySubMeshMaterial.SetFloat("_IsGray",1);
                }
                material = s_GraySubMeshMaterial;
                m_IsGray = true;
            }
            else if(m_CacheMaterial && m_IsGray)
            {
