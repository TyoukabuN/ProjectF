...
namespace TMPro
{
    ...
    public partial class TextMeshProUGUI
    {
        ...
        [SerializeField]
        public bool s_IsCanSortingMask = false;
        ...
        {
            int materialCount = m_textInfo != null ? m_textInfo.materialCount : 1;
        ...
        protected override void GenerateTextMesh()
        {
            ...
            for (int i = 0; i < m_TextParsingBuffer.Length && m_TextParsingBuffer[i].unicode != 0; i++)
            {
                ...
                if (m_textElementType == TMP_TextElementType.Character && !isUsingAltTypeface && ((m_FontStyleInternal & FontStyles.Bold) == FontStyles.Bold)) // Checks for any combination of Bold Style.
                {
                    if (m_currentMaterial.HasProperty(ShaderUtilities.ID_GradientScale))
                    {
                        ...
                        style_padding = m_currentFontAsset.boldStyle / 4.0f * gradientScale * ScaleRatio_A;
                ...
                else
                {
                    ...
                    {
                        float gradientScale = m_currentMaterial.GetFloat(ShaderUtilities.ID_GradientScale);
                        style_padding = m_currentFontAsset.normalStyle / 4.0f * gradientScale * ScaleRatio_A;
            ...
            if (m_renderMode == TextRenderFlags.Render && IsActive())
            {
                ...
                m_mesh.uv = m_textInfo.meshInfo[0].uvs1;
                ...
                for (int i = 1; i < m_textInfo.materialCount; i++)
                {
                    ...
                    m_subTextObjects[i].mesh.uv = m_textInfo.meshInfo[i].uvs1;
                    ...
                    var colorArr = m_textInfo.meshInfo[i].colors32;
                    if (m_UseSpriteGradient) 
                    {
                        int k = 1;
                        Color32 color1 = m_SpriteTopColor;
                        Color32 color2 = m_SpriteBottomColor;
                        for (int t = 0; t < colorArr.Length; t++)
                        {
                            var color = (t == k || t == k + 1) ? color1 : color2;
                            if ((t + 1) % 4 == 0)
                                k += 4;
                            colorArr[t] = color;
                        }
                    }
                    m_subTextObjects[i].mesh.colors32 = colorArr;
        ...
        private RectMask2D m_ParentMaskEx;
        ...
        private void UpdateClipParent()
        {
            ...
            if (m_ParentMaskEx != null && (newParent != m_ParentMaskEx || !newParent.IsActive()))
            {
                m_ParentMaskEx.RemoveClippable(this);
                UpdateCull(false);
            }
            ...
            m_ParentMaskEx = newParent;
        }

        private RectMask2D XGetRectMaskForClippable(IClippable clippable)
        {
            List<RectMask2D> rectMaskComponents = TMP_ListPool<RectMask2D>.Get();
            List<Canvas> canvasComponents = TMP_ListPool<Canvas>.Get();
            ...
            TMP_ListPool<RectMask2D>.Release(rectMaskComponents);
            TMP_ListPool<Canvas>.Release(canvasComponents);
