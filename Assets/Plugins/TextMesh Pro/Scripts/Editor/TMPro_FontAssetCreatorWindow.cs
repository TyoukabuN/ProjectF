    ...
    public class TMPro_FontAssetCreatorWindow : EditorWindow
    {
        ...
        void Save_SDF_FontAsset(string filePath)
        {
            ...
            else
            {
                ...
                if (fontAsset.atlasTextures != null && fontAsset.atlasTextures.Length > 0)
                {
                    fontAsset.atlasTextures[0].Resize(m_AtlasWidth, m_AtlasHeight);
                    fontAsset.atlasTextures[0].SetPixels32(m_FontAtlasTexture.GetPixels32());
                    fontAsset.atlasTextures[0].Apply();
                    //DestroyImmediate(fontAsset.atlasTextures[0], true);//为了不丢失引用
                }
                else
                {
                    // Find all Materials referencing this font atlas.
