...
namespace TMPro.EditorUtilities
{
    ...
    public class TMP_SpriteAssetEditor : Editor
    {
        struct UI_PanelState
        {
            ...
            public static bool spriteEmojiTablePanel;
        }

        class EmojiListInfo
        {
            public string emojiName;
            public List<TMP_Sprite> list;
        }



        private static string[] s_UiStateLabel = new string[] { "<i>(Click to collapse)</i> ", "<i>(Click to expand)</i> ", "<i>(Click to expand)</i> " };
        ...
        List<EmojiListInfo> m_EmojiList;
        string m_EmojiSearchPattern;
        List<int> m_EmojiSearchList;
        bool m_IsEmojiSearchDirty;
        ...
        public override void OnInspectorGUI()
        {
            ...
            // SPRITE Emoji TABLE
            #region  Display Sprite Emoji Table
            EditorGUI.indentLevel = 0;
            rect = EditorGUILayout.GetControlRect(false, 24);
            if (GUI.Button(rect, new GUIContent("<b>Sprite Emoji Table</b>", "List of sprite emoji contained in this sprite asset."), TMP_UIStyleManager.sectionHeader))
                UI_PanelState.spriteEmojiTablePanel = !UI_PanelState.spriteEmojiTablePanel;

            GUI.Label(rect, (UI_PanelState.spriteEmojiTablePanel ? "" : s_UiStateLabel[2]), TMP_UIStyleManager.rightLabel);
            if (UI_PanelState.spriteEmojiTablePanel)
            {
                int arraySize = m_EmojiList.Count;
                int itemsPerPage = 10;
                EditorGUILayout.BeginVertical(EditorStyles.helpBox, GUILayout.ExpandWidth(true));
                {
                    // Search Bar implementation
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        EditorGUIUtility.labelWidth = 110f;
                        EditorGUI.BeginChangeCheck();
                        string searchPattern = EditorGUILayout.TextField("Sprite Search", m_EmojiSearchPattern, "SearchTextField");
                        if (EditorGUI.EndChangeCheck() || m_IsEmojiSearchDirty)
                        {
                            if (string.IsNullOrEmpty(searchPattern) == false)
                            {
                                //GUIUtility.keyboardControl = 0;
                                m_EmojiSearchPattern = searchPattern.ToLower(System.Globalization.CultureInfo.InvariantCulture).Trim();

                                // Search Glyph Table for potential matches
                                SearchEmojiTable(m_EmojiSearchPattern, ref m_EmojiSearchList);
                            }
                            else
                                m_EmojiSearchPattern = null;

                            m_IsEmojiSearchDirty = false;
                        }

                        string styleName = string.IsNullOrEmpty(m_EmojiSearchPattern) ? "SearchCancelButtonEmpty" : "SearchCancelButton";
                        if (GUILayout.Button(GUIContent.none, styleName))
                        {
                            GUIUtility.keyboardControl = 0;
                            m_EmojiSearchPattern = string.Empty;
                        }
                    }
                    EditorGUILayout.EndHorizontal();
                    #endregion
                    ...
                    // Display Page Navigation
                    if (!string.IsNullOrEmpty(m_EmojiSearchPattern))
                        arraySize = m_EmojiSearchList.Count;
                ...
                if (arraySize > 0)
                {
                    ...
                    for (int i = itemsPerPage * m_CurrentCharacterPage; i < arraySize && i < itemsPerPage * (m_CurrentCharacterPage + 1); i++)
                    {
                        ...
                        int elementIndex = i;
                        if (!string.IsNullOrEmpty(m_EmojiSearchPattern))
                            elementIndex = m_EmojiSearchList[i];

                        EmojiListInfo listInfo = m_EmojiList[elementIndex];
                        ...
                        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                        {
                            EditorGUI.BeginDisabledGroup(i != m_selectedElement);
                            {
                                ...
                                DrawEmojiList(listInfo);
                                //EditorGUILayout.PropertyField(listInfo);
                            }
                            EditorGUI.EndDisabledGroup();
                        }
                        EditorGUILayout.EndVertical();
        ...
        void DrawEmojiList(EmojiListInfo info)
        {
            ...
            GUILayout.BeginVertical();
            {
                GUILayout.BeginHorizontal("box");
                {
                    ...
                    EditorGUILayout.LabelField("emojiName: " + info.emojiName, GUILayout.Width(150));
                    EditorGUILayout.LabelField("frameCount: " + info.list[0].frameCount, GUILayout.Width(150));



                    EditorGUI.BeginChangeCheck();
                    int fps = EditorGUILayout.DelayedIntField("fps: ", info.list[0].fps, GUILayout.Width(150));

                    if (EditorGUI.EndChangeCheck())
                    {
                        foreach (var sp in info.list)
                            sp.fps = fps;
                    }
                }
                ...
                {
                    Rect rect = GUILayoutUtility.GetRect(50f, 50f, GUILayout.ExpandWidth(false));
                    rect.x += 5;
                    for (int i = 0; i < info.list.Count; i++)
                    {
                        TMP_Sprite sprite = info.list[i];

                        Rect imgRect = rect;
                        imgRect.x = i * rect.width + i * 2 + rect.x;

                        EditorGUI.DrawRect(imgRect, Color.red);

                        EditorGUILayout.Space();
                        Texture tex = m_SpriteAsset.spriteSheet;
                        Rect texCoords = new Rect(sprite.x / tex.width, sprite.y / tex.height, sprite.width / tex.width, sprite.height / tex.height);
                        GUI.DrawTextureWithTexCoords(imgRect, tex, texCoords, false);
                    }
                }
                GUILayout.EndHorizontal();
        ...
        /// <param name="searchPattern"></param>
        /// <returns></returns>
        void SearchEmojiTable(string searchPattern, ref List<int> searchResults)
        {
            if (searchResults == null) searchResults = new List<int>();
            searchResults.Clear();
            ...
            for (int i = 0; i < arraySize; i++)
            {
                EmojiListInfo lsitInfo = m_EmojiList[i];
                ...
                // Check for potential match against name
                string name = lsitInfo.emojiName.ToLower(System.Globalization.CultureInfo.InvariantCulture).Trim();
                if (name.Contains(searchPattern))
                {
                    searchResults.Add(i);
                    continue;
        ...
        void GenEmojiList()
        {
            Dictionary<string, List<TMP_Sprite>> tempDic = new Dictionary<string, List<TMP_Sprite>>();
            for (int i = 0; i < m_SpriteAsset.spriteInfoList.Count; i++)
            {
                string name = m_SpriteAsset.spriteInfoList[i].name;
                ...
                List<TMP_Sprite> list;
                if (!tempDic.TryGetValue(faceName, out list))
                {
                    list = new List<TMP_Sprite>();
                    tempDic.Add(faceName, list);
                }
                list.Add(m_SpriteAsset.spriteInfoList[i]);
            }

            List<string> keys = tempDic.Keys.ToList();
            keys.Sort((string v1, string v2) => { return v1.CompareTo(v2); });

            m_EmojiList = new List<EmojiListInfo>();
            foreach (var key in keys)
            {
                if (tempDic[key].Count > 1)
                {
                    EmojiListInfo info = new EmojiListInfo();
                    info.emojiName = key;
                    info.list = tempDic[key];
                    m_EmojiList.Add(info);
                }
