Shader "LSQ/EffectAchievement/ProjectorAndDecal_2"
{
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            float len = length(objectPos - 0.5);
            float value = smoothstep(0.41, 0.21, len);

            fixed2 dir = 0.5 - (objectPos - 0.24);
            fixed4 col = tex2D(_MainTex, objectPos + 0.24 * value * dir - 0.5 + _Time.xx);

            return lerp(col, fixed4(0,0,0,1), value);
        }
        ENDCG
    }
    }
