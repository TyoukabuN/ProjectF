...
public class TimerManager : MonoBehaviour
{
    ...
    //设置定时器名字好做性能分析
    public static void SetTimerName(int id, string name)
    {
        if (string.IsNullOrEmpty(name))
        {
            //XLogger.WARNING_Format("TimerManager.SetTimerName name is null  id = {0}", id);
            return;
        }

        if (s_AllTimer.ContainsKey(id))
            s_AllTimer[id].name = name;
        else if (s_AddAwait.ContainsKey(id))
            s_AddAwait[id].name = name;
    }
    ...
    {
        if (!InitTimerManager()) return;
        s_TimerManager.StopCoroutine(coroutine);
    }
    ...
    void Update()
    {
        ...
        //FDeltaFPS.Instance.UpdateEx(Time.deltaTime);
    }
